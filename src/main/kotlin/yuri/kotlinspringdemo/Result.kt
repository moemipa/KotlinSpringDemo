...
class Result<T>(statusEnum: HttpStatusEnum, val data: T) {

    val code: Int = statusEnum.code()
    val msg: String = statusEnum.reasonPhraseCN()

}

enum class HttpStatusEnum(val code: Int, val reasonPhraseUS: String, val reasonPhraseCN: String) {
    ...
    UNKNOW_ERROR(600, "Unknow Error", "未知错误");

    val is1xxInformational: Boolean
        get() = type() == INFORMATIONAL
    ...
    val is5xxServerError: Boolean
        get() = type() == SERVER_ERROR

    fun code(): Int {
        return code
    }

    fun reasonPhraseUS(): String {
        return reasonPhraseUS
    }

    fun reasonPhraseCN(): String {
